...
export function DashboardClientLayout({ 
  ...
  const [businessData] = useState(initialBusinessData);
  ...
  const businessName = businessData?.bisName || businessData?.name || "Admin Demo";
  ...
  return (
    <div className="min-h-screen bg-zinc-50 flex flex-col md:flex-row">
      ...
      <div 
        className={cn(
            "flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out pb-32 md:pb-0",
            isCollapsed ? "md:pl-[88px]" : "md:pl-[280px]"
        )}
      >
        {/* Header */}
        <header className="h-16 md:h-20 bg-transparent flex items-center justify-between px-6 md:px-8 sticky top-0 z-30 transition-all">
            ...
            <div className="h-10 md:h-11 pl-1 pr-1 md:pr-4 rounded-xl md:rounded-2xl bg-zinc-50 border border-zinc-100 flex items-center gap-2 md:gap-3 hover:bg-white transition-all cursor-pointer">
              {businessData?.logo ? (
                <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg md:rounded-xl overflow-hidden relative">
                  <Image src={businessData.logo} alt={businessName} fill className="object-cover" />
                </div>
              ) : (
